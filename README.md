...
**Состояние на 17.09.2026:** нижний образец **IN 22.6 подошёл**. Верхний **OUT 22.6
держится слабо** и выпадает от лёгкого касания. Новая серия — **OUT 22.7–23.0**.
Результаты сохранены в [FIT_LOG.md](FIT_LOG.md). Полную проставку пока не печатать:
нижний диаметр обновлён, верхний ещё предстоит выбрать.
...
- [output](output/) — полная проставка, четыре новых верхних образца и проверенный нижний в STL/STEP,
...
Для следующей примерки напечатать:

- Верхние образцы [Ø22,7](output/pin_gauge_OD_22.7.stl),
  [Ø22,8](output/pin_gauge_OD_22.8.stl), [Ø22,9](output/pin_gauge_OD_22.9.stl),
  [Ø23,0](output/pin_gauge_OD_23.0.stl) — следующая серия для плотной посадки в излив.
  Длина хвостовика каждого образца 20 мм, как у полной детали; примерять от меньшего к большему.
- [socket_gauge_ID_22.6.stl](output/socket_gauge_ID_22.6.stl) — подтверждённая посадка на лейку;
  повторно печатать для подбора диаметра не нужно.
...
Старые образцы удалены из текущего набора; при необходимости доступны в истории Git.
